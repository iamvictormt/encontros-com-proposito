    ...
    <Card className="overflow-hidden border border-gray-100 shadow-sm bg-white rounded-2xl py-0 gap-4">
      ...
      <CardContent className="pb-5">
        <div className="flex flex-wrap gap-2 mb-4">
          {tags.map((tag, i) => (
            <Badge key={i} className="bg-primary hover:bg-primary text-white rounded-full px-3 py-1 text-[12px] border-none">
              {tag}
            </Badge>
          ))}
        </div>
        ...
        <p className="text-sm text-muted-foreground mb-5">
          <span >Local:</span> <span className="underline font-bold text-secondary cursor-pointer">{location}</span>
        </p>
