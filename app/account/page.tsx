...
export default function AccountPage() {
  ...
  const getStatusInfo = (status: string) => {
    const config: Record<string, { label: string; icon: any; color: string; bg: string }> = {
      ...
      DELIVERED: { label: "Entregue", icon: Shield, color: "text-brand-green", bg: "bg-brand-green/10" },
      READY_FOR_PICKUP: { label: "Pronto para Retirada", icon: MapPin, color: "text-purple-600", bg: "bg-purple-50" },
        ...
        <div className="grid lg:grid-cols-12 gap-8 sm:gap-12 relative">
          ...
          <div className="lg:col-span-8 space-y-6 sm:space-y-12">
            {user.userCategory === "PREMIUM" ? (
              <div id="pedido" className="glass p-5 sm:p-10 md:p-12 rounded-[2rem] sm:rounded-[3.5rem] border-white/40 shadow-2xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-64 h-64 bg-brand-orange/10 blur-[80px] -mr-32 -mt-32 rounded-full" />
                
                <div className="space-y-6 sm:space-y-8 relative">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="space-y-1 sm:space-y-2">
                      <span className="text-[8px] sm:text-[10px] font-black text-brand-orange uppercase tracking-[0.2em] sm:tracking-[0.3em]">Logística Premium</span>
                      <h3 className="font-black text-xl sm:text-3xl text-brand-black uppercase tracking-tighter">Seu Acessório Premium</h3>
                    </div>
                    {order && (
                      <div className={cn(
                        ...
                        {order.status === "SENT" ? <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4 animate-pulse" /> : <Shield className="w-3 h-3 sm:w-4 sm:h-4" />}
                  ...
                  {isOrderLoading ? (
                    ...
                    <div className="flex flex-col sm:grid sm:grid-cols-2 gap-6 sm:gap-8 bg-brand-black/[0.02] p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-brand-black/5">
                      <div className="space-y-4">
                        <div className="space-y-1">
                          <span className="text-[7px] sm:text-[9px] font-black text-gray-400 uppercase tracking-widest">Produto Solicitado</span>
                          <p className="text-base sm:text-lg font-black text-brand-black uppercase tracking-tight">{order.accessory_type}</p>
                          <p className="text-[8px] sm:text-[10px] font-bold text-brand-orange uppercase tracking-widest">{order.accessory_model || "Modelo Padrão"}</p>
                        </div>
                        <div className="space-y-1 pt-3 sm:pt-4 border-t border-brand-black/5">
                          <span className="text-[7px] sm:text-[9px] font-black text-gray-400 uppercase tracking-widest">Forma de Entrega</span>
                          <p className="text-xs sm:text-sm font-black text-brand-black uppercase tracking-tight">
                            {order.delivery_method === "RESIDENTIAL" ? "Entrega em Domicílio" : "Retirada no Local"}
                          </p>
                        </div>
                      </div>
                      <div className="flex flex-col justify-center items-center sm:items-end text-center sm:text-right space-y-4 mt-2 sm:mt-0">
                        <div className="w-full sm:w-auto p-5 sm:p-6 bg-white rounded-xl sm:rounded-2xl shadow-xl shadow-brand-black/5 flex flex-col items-center sm:items-end">
                           <span className="text-[7px] sm:text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Status Atual</span>
                           <p className={cn("text-xl sm:text-2xl font-black uppercase tracking-tighter", getStatusInfo(order.status).color)}>
                             {getStatusInfo(order.status).label}
                           </p>
                    ...
                    <div className="bg-brand-orange/5 p-8 sm:p-12 rounded-[2rem] border border-brand-orange/10 text-center space-y-6">
                      <div className="space-y-2">
                        <p className="text-sm sm:text-base font-black text-brand-black uppercase tracking-tight">Acessório não solicitado</p>
                        <p className="text-xs sm:text-sm font-medium text-gray-500 px-4 max-w-sm mx-auto leading-relaxed">
                          Você ainda não solicitou seu acessório exclusivo <span className="text-brand-orange font-bold">MeetOff Premium</span>. Garanta o seu agora para viver a experiência completa!
                        </p>
                      </div>
                      <Button asChild className="h-12 sm:h-14 rounded-2xl bg-brand-orange hover:bg-brand-orange/90 text-white font-black uppercase tracking-widest text-[10px] px-10 shadow-xl shadow-brand-orange/20">
                ...
                <div className="flex flex-col md:flex-row gap-6 sm:gap-10 items-center relative">
                  <div className="w-full max-w-[240px] sm:max-w-[280px] md:w-1/3 aspect-[1.58] bg-brand-black rounded-2xl p-4 sm:p-6 flex flex-col justify-between shadow-2xl rotate-2 group-hover:rotate-0 transition-transform duration-700">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[7px] sm:text-[8px] font-mono text-white/40 uppercase tracking-widest">Cartão MeetOff</p>
                    </div>
                  </div>

                  <div className="flex-1 space-y-4 sm:space-y-6 text-center md:text-left">
                    <div className="space-y-1 sm:space-y-2">
                      <h3 className="font-black text-xl sm:text-3xl text-brand-black uppercase tracking-tighter leading-tight">Seu Acesso Premium</h3>
                    ...
                    <Button asChild className="h-12 sm:h-14 rounded-2xl bg-brand-green hover:bg-brand-green/90 text-white font-black uppercase tracking-widest text-[9px] sm:text-[10px] px-6 sm:px-8 shadow-xl shadow-brand-green/20 group w-full sm:w-auto">
                      <Link href="/member-card" className="flex items-center justify-center gap-2 sm:gap-3">
            ...
            <div className="glass p-5 sm:p-10 rounded-[2rem] sm:rounded-[3rem] border-white/40 shadow-xl space-y-6 sm:space-y-10">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <h3 className="font-black text-base sm:text-xl text-brand-black uppercase tracking-tight">Dados de Segurança</h3>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6 sm:gap-10">
                <div className="flex items-center gap-4 sm:gap-6 group">
                  ...
                  <div className="space-y-0.5 min-w-0">
                    <p className="text-[7px] sm:text-[9px] font-black text-gray-400 uppercase tracking-widest">E-mail Principal</p>
                    <p className="text-brand-black font-black uppercase tracking-tight text-xs sm:text-base truncate">{user.email}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 sm:gap-6 group">
                  ...
                  <div className="space-y-0.5">
                    <p className="text-[7px] sm:text-[9px] font-black text-gray-400 uppercase tracking-widest">Nascimento</p>
                ...
                >
                  ...
                  <div className="space-y-0.5">
                    <p className="text-[7px] sm:text-[9px] font-black text-gray-400 uppercase tracking-widest">Segurança</p>
                    <p className="text-brand-black font-black uppercase tracking-tight text-xs sm:text-base">Alterar Minha Senha</p>
                  </div>
                  <ArrowRight size={14} className="ml-auto text-brand-black/20 group-hover:text-brand-orange group-hover:translate-x-2 transition-all hidden xs:block" />
                </div>
              </div>
              ...
              <div className="pt-6 sm:pt-8 border-t border-brand-black/5">
                <p className="text-[8px] sm:text-[10px] text-gray-400 font-medium italic leading-relaxed">
                  * Suas informações são protegidas por criptografia e nunca serão compartilhadas sem seu consentimento.
                </p>
              </div>
            </div>
          </div>
